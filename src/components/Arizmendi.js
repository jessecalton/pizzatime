...
class Arizmendi extends Component {
  ...
  sanitizePizzaMenu = (pizza) => {
    ...
    try {
      ...
      formattedPizza = "Arizmendi's is closed...";
  ...
  async componentDidMount() {
    try {
      const taste = await axios.get(
        proxyurl + 'https://www.arizmendibakery.com/pizza'
      );
      let pizzaStuffHTML = taste.data;
      this.sanitizePizzaMenu(pizzaStuffHTML);
    } catch (err) {
      this.setState({
        todaysPizza: 'Could not get pizza...',
        pizzaLoaded: false,
      });
    }
  }
  ...
  render() {
    ...
    if (this.state.pizzaLoaded) {
      emoji = (
        <span aria-label='savouring food' role='img'>
          {' '}
          😋
        </span>
      );
    ...
    return (
      <div>
        <h4 className='ideas-header'>
          Need ideas? Today, which is {this.getTodaysDate()},{' '}
          <a href='https://www.arizmendibakery.com/pizza'>Arizmendi's Pizza</a>{' '}
          is serving:
        </h4>
        <div className='ideas'>
          {this.state.todaysPizza}
          {emoji}
        </div>
        <div>
          <span
            className='pointer-emojis'
          ...
          <span
            className='tooltip'
            data-tip="This app doesn't actually order pizza. You should go get it from Arizmendi Bakery."
          >
            Hover over me
          </span>
          <span
            className='pointer-emojis'
            aria-label='pointing left'
            role='img'
          >
            👈
          </span>
